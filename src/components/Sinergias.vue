...
<div class="btn">
  <v-btn elevation="6" class="mb-3 green white--text" @click="gravar">Gravar</v-btn>
  <v-btn class="mb-3 blue white--text" @click="mostrar">Mostrar Dados</v-btn>
  <v-btn class="mb-6 orange white--text"  @click="logout()">Logout</v-btn>
</div>
<hr>
<v-card
max-width="354"
class="mt-4 purple--text card"
color="#ddd">
<v-card-title>Dados das Sinergias</v-card-title>
<ul class="list-group">
  <li  
  v-for="(sinergia, id) in sinergiasDados"
  :key="id"
  >
  ...
  {{ sinergia.obs }}
  <br />
  <v-btn elevation="6" class="btn yellow blue--text mb-3 mt-3" @click="carregar(id)">Carregar Dados</v-btn>
  <v-btn elevation="6" class="btn red white--text mb-3" @click="excluir(id)">Deletar</v-btn>
</li>
</ul>
</v-card>
</v-form>
